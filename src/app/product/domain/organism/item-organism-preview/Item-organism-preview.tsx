    ...
    <div className="d-flex">
      ...
      <div className="w-50">
        ...
        <div className="row w-50">
          {shoe?.tallas?.map((size) => (
            <div onClick={()=> sizeSelectedHandle(size)} key={size} className='mx-1 my-2 col-2'>
              <Button text={size} className={sizeSelected === size ? 'button-sizes-active fs-5 fw-bold scale' : 'button-sizes fs-5 fw-bold scale'}/>
            </div>
          ))}
        </div>
        <div className="d-flex">
        ...
        <div className="d-flex align-items-center mt-4">
          <Button onClick={()=>{addProduct(shoe?.precio)}} text={DICTIONARY.addToCar.toUpperCase()} className='text-white border border-0 bg-color-1 px-5 py-2 scale' />
          <Icon onClick={()=>addFavHandle()} icon={ICONS_CONSTANTS.HEART_FILL} iconColor={favorite ? 'danger cursor-pointer transition' : 'color-2 cursor-pointer transition'} fs="3 ms-5"/>
        </div>
      </div>
    </div>}
    </>
